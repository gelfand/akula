use super::{
    ...
    continuation::{interrupt_data::InterruptData as StateInterruptData, resume_data::ResumeData},
    evm::AnalyzedCode,
    precompiled,
...
use std::{cmp::min, convert::TryFrom, ops::Generator};
...
struct Evm<'state, 'tracer, 'analysis, 'h, 'c, 't> {
    state: &'state mut IntraBlockState,
...
impl<'state, 'tracer, 'analysis, 'h, 'c, 't> Evm<'state, 'tracer, 'analysis, 'h, 'c, 't> {
    fn create(
        &mut self,
        message: CreateMessage,
    ) -> impl Generator<ResumeData, Yield = StateInterruptData, Return = Output> + '_ {
        move |_| {
            let mut res = Output {
            ...
            if gen_await!(self.state.get_balance(message.sender)) < value {
                res.status_code = StatusCode::InsufficientBalance;
                return Ok(res);
            }

            let nonce = gen_await!(self.state.get_nonce(message.sender));
            gen_await!(self.state.set_nonce(message.sender, nonce + 1));
            ...
            if gen_await!(self.state.get_nonce(contract_addr)) != 0
                || gen_await!(self.state.get_code_hash(contract_addr)) != EMPTY_HASH
            {
            ...
            gen_await!(self.state.create_contract(contract_addr));

            if self.block_spec.revision >= Revision::Spurious {
                gen_await!(self.state.set_nonce(contract_addr, 1));
            }

            gen_await!(self.state.subtract_from_balance(message.sender, value));
            gen_await!(self.state.add_to_balance(contract_addr, value));
            ...
            res =
                gen_await!(self.execute(deploy_message, message.initcode.as_ref().to_vec(), None));
            ...
            if res.status_code == StatusCode::Success {
                    ...
                    && code_len > param::MAX_CODE_SIZE
                {
                    ...
                    gen_await!(self.state.set_code(contract_addr, res.output_data.clone()));
    ...
    fn call(
        ...
        move |_| {
            ...
            if message.kind != CallKind::DelegateCall
                && gen_await!(self.state.get_balance(message.sender)) < value
            {
                res.status_code = StatusCode::InsufficientBalance;
                return Ok(res);
            }
            ...
            let code = if !precompiled {
                gen_await!(self.state.get_code(message.code_address))
            } else {
                None
            };
            ...
            if value == 0
                ...
                && !precompiled
                && !gen_await!(self.state.exists(message.code_address))
            {
                return Ok(res);
            }
            ...
            if message.kind == CallKind::Call {
                if message.is_static {
                    ...
                    gen_await!(self.state.subtract_from_balance(message.sender, value));
                    gen_await!(self.state.add_to_balance(message.recipient, value));
                }
            }
            ...
            if precompiled {
                ...
                let code_hash = gen_await!(self.state.get_code_hash(message.code_address));

                res = gen_await!(self.execute(message, code.as_ref().to_vec(), Some(code_hash)));
            }
    ...
    ) -> impl Generator<ResumeData, Yield = InterruptData, Return = Output> + '_ {
        move |_| {
            let a;
            ...
            let output = loop {
                interrupt = match interrupt {
                    ...
                    Interrupt::GetStorage {
                        ...
                        let value = gen_await!(self.state.get_current_storage(address, location));
                    ...
                    Interrupt::SetStorage {
                        ...
                        let current_val =
                            gen_await!(self.state.get_current_storage(address, location));

                        let status = if current_val == new_val {
                            StorageStatus::Unchanged
                        } else {
                            gen_await!(self.state.set_storage(address, location, new_val));
                            ...
                            if !eip1283 {
                                ...
                                let original_val =
                                    gen_await!(self.state.get_original_storage(address, location));
                    ...
                    Interrupt::GetCodeHash { interrupt, address } => {
                        let hash = h256_to_u256({
                            if gen_await!(self.state.is_dead(address)) {
                                H256::zero()
                            } else {
                                gen_await!(self.state.get_code_hash(address))
                            }
                        });
                    ...
                    Interrupt::CopyCode {
                        ...
                        let code = gen_await!(self.state.get_code(address)).unwrap_or_default();
                    ...
                    Interrupt::Selfdestruct {
                        ...
                        let balance = gen_await!(self.state.get_balance(address));
                        gen_await!(self.state.add_to_balance(beneficiary, balance));
                        gen_await!(self.state.set_balance(address, 0));
                    ...
                    Interrupt::Call {
                        ...
                        let output = match call_data {
                            Call::Create(message) => {
                                let mut res = gen_await!(self.create(message));
                            ...
                            Call::Call(message) => gen_await!(self.call(message)),
                        };
                    ...
                    Interrupt::GetBlockHash {
                        ...
                        for i in 1..distance {
                            hash = ResumeData::into_header(
                                yield InterruptData::ReadHeader {
                                    block_number: BlockNumber(base_number.0 - i),
                                    hash,
                                },
                            )
                            .unwrap()
...
mod tests {
    use super::*;
    use crate::{
        execution::continuation::interrupt::StartedInterrupt, res::chainspec::MAINNET,
    ...
    fn execute(
        ...
        txn: &MessageWithSender,
        gas: u64,
    ) -> impl Generator<ResumeData, Yield = InterruptData, Return = CallResult> {
        super::execute(
            state,
            None,
    ...
    #[test]
    fn value_transfer() {
        IntraBlockState::new().execute(StartedInterrupt::from(|_| {
            ...
            let mut state = IntraBlockState::new();

            assert_eq!(gen_await!(state.get_balance(sender)), 0);
            assert_eq!(gen_await!(state.get_balance(to)), 0);
            ...
            let res = gen_await!(execute(&mut state, &header, &txn, gas));
            assert_eq!(res.status_code, StatusCode::InsufficientBalance);
            assert_eq!(res.output_data, vec![]);

            gen_await!(state.add_to_balance(sender, ETHER));

            let res = gen_await!(execute(&mut state, &header, &txn, gas));
            assert_eq!(res.status_code, StatusCode::Success);
            assert_eq!(res.output_data, vec![]);

            assert_eq!(gen_await!(state.get_balance(sender)), ETHER - value);
            assert_eq!(gen_await!(state.get_balance(to)), value);
        }))
    }
    ...
    fn smart_contract_with_storage() {
        IntraBlockState::new().execute(StartedInterrupt::from(|_| {
            ...
            let gas = 0;
            let res = gen_await!(execute(&mut state, &header, &t, gas));
            ...
            let key0 = 0.as_u256();
            assert_eq!(
                gen_await!(state.get_current_storage(contract_address, key0)),
                0x2a
            );

            let new_val = 0xf5.as_u256();

            let res = gen_await!(execute(
                &mut state,
                &header,
                &(txn)(
    ...
    fn maximum_call_depth() {
        IntraBlockState::new().execute(StartedInterrupt::from(|_| {
            ...
            gen_await!(state.set_code(contract, code.to_vec().into()));
    ...
    #[test]
    fn delegatecall() {
        InMemoryState::default().execute(StartedInterrupt::from(Box::pin(|_| {
            let header = PartialHeader {
            ...
            gen_await!(state.set_code(caller_address, caller_code.to_vec().into()));
            gen_await!(state.set_code(callee_address, callee_code.to_vec().into()));
            ...
            let key0 = 0.as_u256();
            assert_eq!(
                gen_await!(state.get_current_storage(caller_address, key0)),
